...
namespace JSONCode
{
	...
	public class JSON
	{
		...
		protected static bool SerializeValue(object value, StringBuilder builder)
		{
            ...
            // Utilities specific
            // calc specific
            else if (value is Calculator.MemoryItem)
                success = SerializeMemoryObject((Calculator.MemoryItem)value, builder);

            // pcal specific
            else if (value is PCalender.Period)
                success = SerializePeriodObject((PCalender.Period)value, builder);

            // TODO add code for other apps
        ...
        private static bool SerializeMemoryObject(Calculator.MemoryItem memObject, StringBuilder builder)
        {
            builder.Append("{");

            // Serialize the Text
            SerializeString(Utilities.StateManager.CalcState.Text.ToString(), builder);
            builder.Append(":");
            if (!SerializeString(memObject.Text, builder))
                return false;

            builder.Append(", ");

            // Serialize the Value
            SerializeString(Utilities.StateManager.CalcState.Value.ToString(), builder);
            builder.Append(":");
            if (!SerializeString(memObject.Value, builder))
                return false;

            builder.Append(", ");

            // Serialize the type
            SerializeString(Utilities.StateManager.CalcState.Type.ToString(), builder);
            builder.Append(":");
            if (!SerializeNumber((int)memObject.Type, builder))
                return false;
		...
		protected static bool SerializeObject(Dictionary<string,object> anObject, StringBuilder builder)
		{
			...
			Dictionary<string,object>.Enumerator e = anObject.GetEnumerator();
			bool first = true;
			while (e.MoveNext()) {
